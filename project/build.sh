...
function choose_target_board() {
	...
	LUNCH_NUM=$((LUNCH_NUM + 1))
	echo "${space8}${space8}[${LUNCH_NUM}] RV1106_PangHuV1"
	LUNCH_NUM=$((LUNCH_NUM + 1))
	echo "${space8}${space8}[${LUNCH_NUM}] RV1106_Echo_Mate-DeskMate"
	LUNCH_NUM=$((LUNCH_NUM + 1))
	echo "${space8}${space8}[${LUNCH_NUM}] RV1106_Core_Master"
	LUNCH_NUM=$((LUNCH_NUM + 1))
	echo "${space8}${space8}[${LUNCH_NUM}] RV1106_Master"
	LUNCH_NUM=$((LUNCH_NUM + 1))
	...
	range_sd_card=(0)
	range_sd_card_spi_nand=(1 2 3 4 12 13 14 15)
...
function _SAVE_UBOOT_CONFIG() {
	...
	cp ${UBOOT_PATH}/configs/*panghu* ${SDK_SYSDRV_DIR}/tools/board/uboot/
	cp ${UBOOT_PATH}/configs/*echo* ${SDK_SYSDRV_DIR}/tools/board/uboot/
	cp ${UBOOT_PATH}/arch/arm/dts/*luckfox* ${SDK_SYSDRV_DIR}/tools/board/uboot/
	cp ${UBOOT_PATH}/arch/arm/dts/*panghu* ${SDK_SYSDRV_DIR}/tools/board/uboot/
	cp ${UBOOT_PATH}/arch/arm/dts/*echo* ${SDK_SYSDRV_DIR}/tools/board/uboot/
}
...
function _SAVE_KERNEL_CONFIG() {
	...
	cp ${KERNEL_PATH}/arch/arm/configs/*panghu* ${SDK_SYSDRV_DIR}/tools/board/kernel/
	cp ${KERNEL_PATH}/arch/arm/configs/*echo* ${SDK_SYSDRV_DIR}/tools/board/kernel/
	cp ${KERNEL_PATH}/arch/arm/boot/dts/*luckfox* ${SDK_SYSDRV_DIR}/tools/board/kernel/
	cp ${KERNEL_PATH}/arch/arm/boot/dts/*panghu* ${SDK_SYSDRV_DIR}/tools/board/kernel/
	cp ${KERNEL_PATH}/arch/arm/boot/dts/*echo* ${SDK_SYSDRV_DIR}/tools/board/kernel/
}
...
function _SAVE_BUILDROOT_CONFIG() {
	if [ -d ${BUILDROOT_PATH} ]; then
		...
		cp ${BUILDROOT_PATH}/configs/*panghu* ${SDK_SYSDRV_DIR}/tools/board/buildroot
		cp ${BUILDROOT_PATH}/configs/*echo* ${SDK_SYSDRV_DIR}/tools/board/buildroot
		cp -f ${BUILDROOT_PATH}/configs/*rv1106_master* ${SDK_SYSDRV_DIR}/tools/board/buildroot/ 2>/dev/null || true
	fi
}

# sysdrv Makefile 在每次 make buildroot 前会把 tools/board 下的 defconfig 拷回 buildroot/configs/；
# 若 menuconfig 后不同步回 tools/board，下次全编会用旧 defconfig（例如 iperf3 等选项会“丢失”）。
function __SYNC_BUILDROOT_DEFCONFIG_TO_TOOLS_BOARD() {
	if [ -z "$RK_BUILDROOT_DEFCONFIG" ]; then
		msg_warn "RK_BUILDROOT_DEFCONFIG 为空，未同步 Buildroot defconfig 到 tools/board"
		return
	fi
	local def="${BUILDROOT_PATH}/configs/${RK_BUILDROOT_DEFCONFIG}"
	if [ ! -f "$def" ]; then
		msg_warn "找不到 ${def}，跳过同步"
		return
	fi
	cp -f "$def" "${SDK_SYSDRV_DIR}/tools/board/buildroot/${RK_BUILDROOT_DEFCONFIG}"
	msg_info "已同步 Buildroot defconfig -> tools/board/${RK_BUILDROOT_DEFCONFIG}"
}
	...
	if [ "${LF_TARGET_ROOTFS}" == "buildroot" ]; then
		...
		if [ -f $BUILDROOT_CONFIG_FILE ]; then
			...
			if [ "$BUILDROOT_CONFIG_FILE_MD5" != "$BUILDROOT_CONFIG_FILE_NEW_MD5" ]; then
				...
				__SYNC_BUILDROOT_DEFCONFIG_TO_TOOLS_BOARD
			fi
		else
			make buildroot_menuconfig -C ${SDK_SYSDRV_DIR}
